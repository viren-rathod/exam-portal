package org.examportal.Constants;

public class UserMessages {
    public static final String USER_EXIST = "Username already exists!";
    public static final String USER_FOUND = "User found!";
    public static final String USER_NOT_FOUND = "User not found!";
    public static final String EMAIL_EXIST = "E - mail already exists!";
    public static final String REGISTERED = "Registered successfully!";
    public static final String LOGGED = "Login Successfully!";
    public static final String NO_CONTENT = "No Content!";
    public static final String CANDIDATE_SAVED = "Candidate Saved Successfully!";
    public static final String COLLEGE_ADDED = "College Saved!";
    public static final String ALL_COLLEGE_ADDED = "All Colleges Saved!";
}
